...
float LIMIAR_LIGAR = 30.0;    // Ligar irrigador se abaixo de 30%
float LIMIAR_DESLIGAR = 40.0; // Desligar irrigador se acima de 40%
int seco = 4095;              // Valor ADC para solo seco
int molhado = 1850;           // Valor ADC para solo molhado
...
<head>
  ...
  <style>
    html { font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 20px; text-align: center; }
    h2 { color: #2c3e50; margin-bottom: 30px; }
    .card { background: white; border-radius: 10px; padding: 20px; margin: 15px auto; max-width: 400px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
    ...
    .label { font-size: 1.1rem; color: #34495e; margin-bottom: 5px; }
    .progress-container { width: 100%; background-color: #ecf0f1; border-radius: 5px; margin: 15px 0; }
    .progress-bar { height: 20px; border-radius: 5px; background-color: #3498db; width: 0%; transition: width 0.5s; }
  </style>
</head>
<body>
...
function updateData() {
  ...
  fetch("/raw").then(r => r.text()).then(t => {
    document.getElementById("rawValue").textContent = t;
  });
}
